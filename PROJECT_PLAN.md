...
| M8        | Integrations & Imports | Week 16    | In Progress | User-requested (2026-09-24): CRM import (HubSpot) + calendar connections (Google Calendar, Calendly) + calendar import. `20260924000016_integrations.sql` + shared lib landed; feature work in parallel; paste + live verify pending                                    |
...
- [ ] **HubSpot CRM import** — OAuth connect + callback, paginated contact fetch, standard-field mapping (email/phone/name/company), duplicate handling identical to CSV import (§37: flag, never merge), results summary; surfaced on `/dashboard/integrations`
- [ ] **Calendar connections** — Google Calendar (OAuth, `calendar.readonly`) + Calendly (PAT) connect/disconnect managed on `/dashboard/calendar`
- [ ] **Calendar import** — pull events into `imported_calendar_events`, merged month view (appointments + imported events + blocked times), re-import + clear controls
- [ ] **Env gating** — providers degrade to disabled "not configured" cards until their keys are set (`HUBSPOT_CLIENT_*`, `GOOGLE_CAL_CLIENT_*`, `CALENDLY_API_KEY`)
